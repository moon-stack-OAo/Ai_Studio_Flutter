...
- 本地升版：`node .github/scripts/bump-version.mjs 1.0.2`（可选 `--build <n>` / `--dry-run`）；只改双端 `pubspec`，并自动跑 `sync-changelog-asset`；不自动 commit/tag。
- 改完根目录 `CHANGELOG.md` 后构建前：`node .github/scripts/sync-changelog-asset.mjs`（同步到 `packages/core/assets/CHANGELOG.md`，关于页运行时读取）。
